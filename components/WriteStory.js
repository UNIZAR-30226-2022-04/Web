...
router.push("/login")
  ...
  useEffect(()=>{
    ...
    const getPrevious = async () =>{
      if(!first){        
        ...
        const res = await fetch("http://localhost:3000/api/tale_mode/resume_tale", options)
  ...
  const add_tale_paragraph = async () => {
    ...
    const res = await fetch("http://localhost:3000/api/tale_mode/add_tale_paragraph", options)
    const data = await res.json()
    return data
  }
  ...
  return (
    <div className="flex flex-row w-full mt-10 p-10 items-start">
                ...
                return(
                  <div className="w-full">
                    ...
                    <div className="flex flex-row space-x-1">
                      ...
                      <div className="font-arial-b">{parrafo.username}</div>
                    </div>
                    
                    <div className="bg-verde_parrafo text-white font-arial-b p-2 overflow-visible">{parrafo.text}</div>
                  </div>
                )
              })}
            </>
      ...
      <form className="flex flex-col items-center w-1/2 h-3/4 space-y-3 pt-32" onSubmit={onSubmit}>
        <div className="commonTitle">
          {first ? "Empieza tu relato":currentTitle}
        </div>
        
        {first ? (
          <input className="text-center text-6xl font-arial-b rounded-xl" type="text" required={true} placeholder="Inserte título del relato" onChange={(e) => setCurrentTitle(e.target.value)} />
        ):(
          <></>
        )}          
        ...
        <textarea disabled={disabled} className="border-blue-800 border-4 rounded-2xl text-center text-lg flex-col h-1/2 w-full p-3" type="text" required={true} maxLength={storyInfo.maxChar} placeholder={placeh} onChange={(e) => setCurrentText(e.target.value)}/>
        ...
        <div className="flex flex-row space-x-3">
          {!lastWrite ? (
            ...
            <div className="flex flex-col">
              <div className="font-arial-b">Has sido el último en escribir</div>
              <button className="commonButton bg-red-500 shadow-red-800" type="button" onClick={(e) => (router.push('/storyMode'))}>{'<-'}Volver</button>
            </div>
            
          )}

          {(!first && (storyInfo.creator == windowUser.username)) ? (
            <button className="commonButton bg-red-500 shadow-red-800" type="button" onClick={(e) => (finishTale(windowUser, storyInfo, currentText,router ))}>Terminar relato</button>
          ):(
            <></>
          )}
        </div>
